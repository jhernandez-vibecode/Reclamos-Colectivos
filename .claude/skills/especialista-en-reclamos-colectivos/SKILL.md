...
**Última actualización:** 19 ago 2026 — botón de eliminar reclamo + lápidas en el autoseed + corrección del monto asegurado del caso #58222025000408.
...
- **🔑 El monto asegurado NO es el monto pagado.** En la VTM 805 el asegurado es siempre uno de los 5 escalones fijos (2/4/6/8/10M) y el pagado sale un poco menor (₡10.000.000 → ₡9,8–9,9M por ajustes del INS). **Si un caso tiene `montoAseg === montoInd` y el valor no es redondo, es un error de digitación**: se metió el pagado en la casilla del asegurado. Corregido así el 19 ago 2026 en el caso #58222025000408 (`s27`, era ₡9.885.430 → ₡10.000.000, conservando el pagado). Ver abajo cómo lo trata el gráfico.
- **🔴 El gráfico por monto asegurado NO tiene categoría "Otro" (decisión de JC, 19 ago 2026).** Los montos asegurados de la póliza son **solo** sus escalones fijos, así que cualquier valor distinto se cuenta en el **escalón más cercano** (`Math.abs` mínimo) en vez de mandarse a una tajada aparte. Así ningún caso se pierde del gráfico y el total del gráfico siempre cuadra con la cantidad de casos del año. Los casos con monto 0 o vacío siguen sin contarse (no tienen escalón). El select del modal tampoco escribe "(otro)": muestra el monto tal cual, para que un valor mal digitado se vea y se pueda corregir a mano.
- ⚠️ **Consecuencia asumida:** al no existir "Otro", un monto mal digitado ya **no salta a la vista en las estadísticas** — se absorbe en el escalón vecino. JC lo sabe y lo prefiere así. Si en el futuro hace falta detectar esos casos, el chequeo es `montoAseg === montoInd` o `montoInd > montoAseg` (ver el punto anterior), no el gráfico.
- 🟡 Ojo: el `Otro` que queda en `app.html:830` es el **vínculo del afectado** (parentesco: Titular/Cónyuge/Hijo-a/…/Otro), no un monto. Ese no se toca.
- 🟠 **PENDIENTE de confirmar con JC / el INS:** el caso **#58222025000342** (`s19`, ANCHIA BLANCO ELI GERARDO, 2025) tiene asegurado ₡6.000.000 y pagado ₡9.852.696 — **pagar más que el asegurado es imposible**. El pagado calza exactamente con el patrón de los casos de ₡10M, así que lo más probable es que el asegurado deba ser ₡10.000.000, pero no se cambió sin confirmación. Menos grave: #58222025000435 paga ₡1.901.797 sobre ₡10M (19 %). Los tres de VTM 703 al 50 % son consistentes entre sí y parecen legítimos.
- 🔴 **Corregir el SEED NO corrige producción.** El backfill del GET solo **agrega** los ids que faltan; nunca actualiza los que ya están en el blob. Un arreglo de dato en el SEED solo aplica a la demo local y a un blob vacío. Para producción hay que editar el caso desde la app (o migrar el blob a mano).
